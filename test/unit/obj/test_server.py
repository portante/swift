...
class TestObjectController(unittest.TestCase):
    ...
    def test_HEAD_quarantine_zbyte(self):
        ...
        file_name = os.path.basename(disk_file.data_file)
        with open(disk_file.data_file) as fp:
            metadata = diskfile.read_metadata(fp)
        os.unlink(disk_file.data_file)
        with open(disk_file.data_file, 'w') as fp:
            diskfile.write_metadata(fp, metadata)

        file_name = os.path.basename(disk_file.data_file)
        self.assertEquals(os.listdir(disk_file.datadir)[0], file_name)
        req = Request.blank('/sda1/p/a/c/o',
                            environ={'REQUEST_METHOD': 'HEAD'})
        resp = req.get_response(self.object_controller)
        self.assertEquals(resp.status_int, 404)
        ...
        quar_dir = os.path.join(
            self.testdir, 'sda1', 'quarantined', 'objects',
            os.path.basename(os.path.dirname(disk_file.data_file)))
        self.assertEquals(os.listdir(quar_dir)[0], file_name)
    ...
    def test_GET_quarantine(self):
        ...
        diskfile.write_metadata(disk_file.fp, metadata)
        self.assertEquals(os.listdir(disk_file.datadir)[0], file_name)
        req = Request.blank('/sda1/p/a/c/o')
        resp = req.get_response(self.object_controller)
        quar_dir = os.path.join(
