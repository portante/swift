...
class InMemoryFileSystem(object):
    """
    This is a very simplistic scheme. There is one dictionary mapping a given
    object name to a tuple. The first entry in the tuble is the cStringIO
    buffer representing the file contents, the second entry is the metadata
    dictionary.
    """
...
class DiskFileReader(object):
    ...
    def close(self, verify_file=True):
        ...
        if self._fp:
            try:
                if verify_file and self._started_at_0 and self._read_to_eof:
...
class DiskFile(object):
    """
    Manage object files in-memory. This duck-types the standard DiskFile class.
